...
class ImageWriter:
    ...
    def __init__(self, id, out_file_loc):
...
def multiple_avi_to_bmp(in_file_dir: str, out_file_dir: str, preprocess_func=None):
    for subdirs, dirs, files in os.walk(in_file_dir):
        for file in files:
            avi_to_bmp(os.path.join(in_file_dir, file), out_file_dir, preprocess_func)


def avi_to_bmp(in_file_loc: str, out_file_dir: str, preprocess_func=None):
    augment_video(in_file_loc, out_file_dir, preprocess_func, save_as_images=True)


def augment_video(in_file_loc: str, out_file_dir: str, preprocess_func=None, fourcc='XVID', save_as_images=False):
    ...
    if save_as_images:
        out = ImageWriter(os.path.basename(in_file_loc), out_file_dir)
    else:
        out = cv.VideoWriter(
...
def final(in_file_dir, out_file_dir, preprocess_func=None):
